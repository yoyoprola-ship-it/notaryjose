...
export default function OwnerHoursPage() {
  const [config, setConfig] = useState<WorkingHours>({ hoursByDayOfWeek: {}, blockedDates: [] });
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (res.ok && json.data) setConfig(json.data as WorkingHours);
      } catch (err) {
        console.error('[owner/hours] load failed:', err);
      } finally {
        setLoading(false);
      }
    })();
  ...
  const toggleHour = (dow: number, h: number) => {
    ...
    setConfig({
      ...config,
      hoursByDayOfWeek: { ...(config.hoursByDayOfWeek || {}), [dow]: next },
    });
    setSaved(false);
  };
