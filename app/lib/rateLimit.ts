// Token bucket rate limiter con backend en Firestore (colección shared).
// Scope prefix `nj-*` para no colisionar con rudewear (`rw-*`) o toolhome
// (`th-*`). Fallback in-memory con cap para no OOM en outage de Firestore.
  ...
  try {
    ...
    return await adminDb.runTransaction(async (tx) => {
      ...
      if (!snap.exists) {
        ...
        tokens = Math.min(
          config.maxRequests,
          (data.tokens || 0) + elapsed * refillRate
        );
        lastRefill = now;
      }
      if (tokens < 1) {
