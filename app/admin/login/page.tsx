...
export default function AdminLoginPage() {
  ...
  const handleSendSms = async (e: React.FormEvent) => {
    ...
    if (digits.length !== 10) {
      setError('Enter a valid 10-digit US phone.'); return;
    }
    userInitiated.current = true;
    setLoading(true);
    try {
  ...
  const requestEmailCode = async (): Promise<void> => {
    ...
    try {
      ...
      if (!res.ok) {
        if (res.status === 429) setError(data.error || 'Slow down.');
        else setError(data.error || `Failed (status ${res.status})`);
        setStep('email2fa');
  ...
  const handleVerifyEmailCode = async (e: React.FormEvent) => {
    e.preventDefault();
    if (loading) return;
    if (emailCode.length !== 6) { setError('Code must be 6 digits.'); return; }
    setLoading(true);
    setError('');
    try {
      const idToken = await auth.currentUser?.getIdToken();
    ...
    <main className="min-h-screen flex items-center justify-center bg-stone-50 text-slate-900 px-6 py-12">
      <div className="w-full max-w-sm">
          ...
          {[0, 1, 2].map((i) => {
            ...
            return (
              <div key={i} className={`h-1 flex-1 rounded ${
                i <= stepIdx ? 'bg-amber-700' : 'bg-stone-200'
              }`} />
            );
          })}
        </div>
        <p className="text-xs text-slate-500 mb-6">
          ...
          {step === 'code' && `Step 2 of 3 — Enter the 6-digit code sent to ${phone}.`}
          {step === 'email2fa' && `Step 3 of 3 — Extra check. Code sent to ${maskedEmail || 'your email'}.`}
        </p>
        ...
        {step === 'email2fa' && (
          <form onSubmit={handleVerifyEmailCode} className="flex flex-col gap-4">
            <input
              ...
              className={`${inputCls} text-center text-2xl font-black tracking-[0.5em]`}
            />
            <p className="text-xs text-slate-500">Check your inbox (and spam). Code expires in 10 min.</p>
            {error && <p className="text-sm text-red-600">{error}</p>}
            <button type="submit" disabled={loading || emailCode.length !== 6} className={btnCls}>
              {loading ? 'Verifying…' : 'Enter admin panel'}
            </button>
            <div className="flex justify-between items-center">
              <button type="button" onClick={handleCancelEmail2fa} disabled={loading}
                className="text-xs text-slate-500 hover:text-slate-800">
                ← Cancel & sign out
              </button>
              <button
                type="button" onClick={() => requestEmailCode()}
        ...
        <div id="notaryjose-admin-recaptcha" ref={recaptchaRef} />
      </div>
    </main>
  );
}
