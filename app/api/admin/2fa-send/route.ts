...
export async function POST(request: NextRequest) {
  ...
  const ipRl = await rateLimitOr429(`nj-admin-2fa-send-ip:${ip}`, {
  ...
  const uidRl = await userRateLimitOr429('nj-admin-2fa-send', caller.uid, {
  ...
  const docRef = adminDb.doc(`notaryjose_adminTwoFactor/${caller.uid}`);
  try {
    ...
    console.warn('[admin/2fa-send] cooldown check failed:', err);
  }
      ...
      body: JSON.stringify({
        ...
        html: `
          <div style="font-family: 'Segoe UI', sans-serif; max-width: 480px; margin: 0 auto; padding: 32px; background: #fafaf9; color: #0f172a;">
            ...
            <p style="color: #64748b; font-size: 13px; margin: 0 0 24px; text-align: center;">Expires in 10 minutes.</p>
            <div style="text-align: center; margin: 24px 0;">
              <span style="display: inline-block; padding: 16px 32px; background: #fffbeb; border: 2px solid #b45309; border-radius: 8px; font-size: 32px; font-weight: 900; letter-spacing: 8px; color: #78350f;">${safeCode}</span>
            </div>
            <p style="color: #b91c1c; font-size: 12px; line-height: 1.5; margin-top: 24px; padding-top: 16px; border-top: 1px solid #e2e8f0;">
              <strong>If you didn&apos;t sign in</strong>, do NOT share this code. Change your phone in Firebase console.
            </p>
          </div>
        `,
      }),
