...
export async function POST(request: NextRequest) {
  ...
  const ipRl = await rateLimitOr429(`nj-admin-2fa-verify-ip:${ip}`, {
  ...
  const uidRl = await userRateLimitOr429('nj-admin-2fa-verify', caller.uid, {
  ...
  const codeDocRef = adminDb.doc(`notaryjose_adminTwoFactor/${caller.uid}`);
  const userDocRef = adminDb.doc(`users/${caller.uid}`);

  const result = await adminDb.runTransaction(async (tx) => {
    const snap = await tx.get(codeDocRef);
    if (!snap.exists) return { error: 'No active 2FA code. Request a new one.', status: 400 };
